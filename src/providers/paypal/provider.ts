...
/**
 * Create an authentication provider for PayPal API
 * 
...
generateHandlers(operations: IParsedEndpoint[], options: IHandlerGenerationOptions): string {
  ...
  const orderOperations = operations.filter(op => op.path.includes('/checkout/orders'));
  const otherOperations = operations.filter(op => !op.path.includes('/checkout/orders'));
  
  const handlerMethods: string[] = [];
  
  // Generate handlers for order operations
  for (const operation of orderOperations) {
    // Convert operationId to method name (e.g., "v2.checkout.orders.create" to "ordersCreate")
    const methodName = this.operationIdToToolName(operation.operationId);
    
    // Determine which template to use based on the operation
    let templateFile = 'operations/default.ts.template';
    
    if (operation.path.endsWith('/orders') && operation.method === 'POST') {
      templateFile = 'operations/orders-create.ts.template';
    } else if (operation.path.match(/\/orders\/\{id\}$/) && operation.method === 'GET') {
      templateFile = 'operations/orders-get.ts.template';
    } else if (operation.path.includes('/capture') && operation.method === 'POST') {
      templateFile = 'operations/orders-capture.ts.template';
    }
    ...
    try {
      ...
      if (fs.existsSync(templatePath)) {
        operationCode = fs.readFileSync(templatePath, 'utf8');
      } else {
        console.warn(`Template file not found: ${templateFile}, using default`);
        operationCode = '      // Default implementation for ${operation.operationId}\n' +
                        '      return { success: true, operation: "${operation.operationId}" };';
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error(`Error loading template file: ${errorMessage}`);
      operationCode = '      // Error loading template\n' +
                      '      return { success: false, error: "Template error" };';
    }
    
    // Load the handler template
    ...
    const handlerCode = handlerTemplate
      .replace(/\${description}/g, operation.description || operation.summary || `${methodName} operation`)
      .replace(/\${methodName}/g, methodName)
      .replace(/\${operationCode}/g, operationCode);
    
    handlerMethods.push(handlerCode);
  }
  
  // Process other operations if needed
...
generateServerImplementation(spec: IParsedSpec, config: IProviderConfig): string {
    ...
    const serverClassName = this.formatServerClassName(config.name || 'paypal-mcp');
    ...
    const serverImpl = serverTemplate
      .replace(/\${serverName}/g, config.name || 'PayPal API')
      .replace(/\${serverVersion}/g, config.version || '1.0.0')
      .replace(/\${serverDescription}/g, config.description || 'PayPal API MCP Server')
    ...
    // Inject helper methods - add them before the last closing brace
    const lastBraceIndex = serverImpl.lastIndexOf('}');
    if (lastBraceIndex !== -1) {
      return serverImpl.substring(0, lastBraceIndex) + 
             '\n\n  ' + helperMethods + '\n' + 
             serverImpl.substring(lastBraceIndex);
    }
   ...
   * Generate tools list handler
   */
  private generateToolsListHandler(operations: IParsedEndpoint[]): string {
    // For minimal implementation, we'll return a very simple tools list
    const tools = this.mapOperationsToTools(operations);
...
generateAdditionalFiles(spec: IParsedSpec, config: IProviderConfig): Map<string, string> {
    ...
    // Load the PayPal types template
    const typesTemplatePath = path.join(__dirname, 'templates', 'paypal-types.ts.template');
    const typesContent = fs.readFileSync(typesTemplatePath, 'utf8');
    
    files.set('src/paypal-types.ts', typesContent);
    
    // Generate package.json additions for PayPal dependencies
