...
describe('PayPalProvider', () => {
  ...
  describe('createAuthProvider', () => {
    test('creates auth provider with default config', () => {
      ...
      expect(result.name).toBe('paypal-auth-provider');
      expect(result.type).toBe('paypal-oauth2');
    });
  ...
  describe('generateServerImplementation', () => {
    test.skip('generates server implementation with correct class name', () => {
      ...
      const config = {
        name: 'paypal-mcp',
        version: '1.0.0',
        description: 'PayPal MCP Server'
      };

      const serverImplementation = provider.generateServerImplementation(parsedSpec, config);

      expect(serverImplementation).toContain('mock template content');
      expect(fs.readFileSync as jest.Mock).toHaveBeenCalled();
      expect(fs.existsSync as jest.Mock).toHaveBeenCalled();
    });

    test('generates handlers for operations', () => {
      const parsedSpec: IParsedSpec = {
        title: 'PayPal API',
        version: '1.0.0',
        description: 'PayPal API',
        ...
        endpoints: [
          createEndpoint({
            ...
            method: 'POST',
            operationId: 'v2/checkout/orders/create'
          }),
          createEndpoint({
            path: '/v2/checkout/orders/{id}',
            method: 'GET',
            operationId: 'v2/checkout/orders/get'
          }),
          createEndpoint({
            path: '/v2/checkout/orders/{id}/capture',
            method: 'POST',
            operationId: 'v2/checkout/orders/capture'
          })
        ],
        securitySchemes: {},
        components: { schemas: {} }
      ...
      provider.generateServerImplementation(parsedSpec, config);

      // Verify specific template for order operations was used
      expect(fs.existsSync).toHaveBeenCalledWith(expect.stringContaining('orders-create.ts.template'));
      expect(fs.existsSync).toHaveBeenCalledWith(expect.stringContaining('orders-get.ts.template'));
      expect(fs.existsSync).toHaveBeenCalledWith(expect.stringContaining('orders-capture.ts.template'));
    });

    test('generates additional files', () => {
      const parsedSpec: IParsedSpec = {
        title: 'PayPal API',
        version: '1.0.0',
        description: 'PayPal API',
      ...
      const additionalFiles = provider.generateAdditionalFiles(parsedSpec, config);

      expect(additionalFiles.size).toBeGreaterThan(0);
      expect(additionalFiles.has('src/paypal-types.ts')).toBe(true);
      expect(additionalFiles.has('package.json.additions')).toBe(true);
      expect(additionalFiles.get('package.json.additions')).toContain('axios');
    });

    test('handles template loading errors', () => {
      // Set up mocks for template error
      ...
      // Override with error-throwing implementation
      mockExistsSync.mockReturnValueOnce(false);
      mockReadFileSync.mockImplementationOnce(() => {
        return 'Error template content';
      });
      ...
      const result = provider.generateServerImplementation(parsedSpec, config);
